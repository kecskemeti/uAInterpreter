...
public class SyntaxCheck implements Visitor {
	private int constantIndex = UAMemory.constants;
	...
	public void visit(IntNumber e) {
		UAMemory.setConstant(constantIndex++, e.containedValue);
	}

	@Override
	public void visit(StringValue e) {
		for (int i = 0; i < e.containedValue.length(); i++) {
			UAMemory.setConstant(constantIndex++, e.containedValue.charAt(i));
		}
	}
	...
	private void visitOp(Operation e) {
		System.out.println("Checking for " + e.myloc);
